...
class GithubRepository @Inject constructor(private val client: HttpClient) {
     ...
     */
    suspend fun searchRepositories(query: String): RepositoryState<List<RepositoryItem>> =
        withContext(Dispatchers.IO) {
            try {
                // GitHub APIのエンドポイントにリクエストを送信し、レスポンスを文字列で取得する。
                val response =
                    client.get<String>("https://api.github.com/search/repositories?q=$query")
                // レスポンスからJSONオブジェクトを作成する。
                val jsonBody = JSONObject(response)
                // JSONオブジェクトからリポジトリの配列を取得する。配列が存在しない場合はエラーを返す。
                val jsonItems =
                    jsonBody.optJSONArray("items") ?: return@withContext RepositoryState.Error(
